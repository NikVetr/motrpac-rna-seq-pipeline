...
ARG MAX_ROUND=20000
COPY dockerfiles/rsem-convergence.patch dockerfiles/rsem_convergence.h /tmp/
RUN curl -fsSL https://github.com/deweylab/RSEM/archive/refs/tags/v1.3.3.tar.gz -o /tmp/rsem.tar.gz \
    ...
    && cd /tmp/RSEM-1.3.3 \
    && patch -p1 < /tmp/rsem-convergence.patch \
    && cp /tmp/rsem_convergence.h . \
    ...
    && cp COPYING EM.cpp SingleHit.h PairedEndHit.h rsem_convergence.h /usr/local/share/rsem/ \
    && rm -rf /tmp/rsem.tar.gz /tmp/RSEM-1.3.3 /tmp/rsem-convergence.patch /tmp/rsem_convergence.h
