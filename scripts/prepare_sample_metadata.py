...
def prepare(matrix, study, qc, output, required, expression_metadata=None):
    with matrix.open(encoding="utf-8") as handle:
        header = next(csv.reader(handle, delimiter="\t"))
    samples = header[1:]
    if header[0] not in ("gene_id", "transcript_id") or not samples or "" in samples or len(samples) != len(set(samples)):
...
def main():
    ...
    prepare(args.matrix, args.study, args.qc, args.output, args.required_study_columns, args.expression_metadata)
