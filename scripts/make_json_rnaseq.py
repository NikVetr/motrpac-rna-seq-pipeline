...
RUNTIME_POLICY_KEYS = {"rnaseq_pipeline.prefer_predefined_n1", "rnaseq_pipeline.use_e2"}
...
def load_runtime_profile(path):
    ...
    unknown = sorted(actual_keys - RUNTIME_RESOURCE_KEYS - RUNTIME_POLICY_KEYS)
    if missing or unknown:
        details = []
        if missing:
            details.append("missing {}".format(", ".join(missing)))
    ...
    for key in RUNTIME_POLICY_KEYS & actual_keys:
        if type(overrides[key]) is not bool:
...
def make_json_dict(
    ...
    filled_dict = {
        ...
        "rnaseq_pipeline.rsem_docker": "us-docker.pkg.dev/motrpac-portal/rnaseq/rsem@sha256:80120bc60cdf1756bc7073c070e18c607450c3f2d3cd1ac9cc3ee49b43a3c3a7",
        ...
        "rnaseq_pipeline.collect_qc_docker": "us-docker.pkg.dev/motrpac-portal/rnaseq/collect_qc@sha256:d4180349cae47041cf003aa960151d0342da2c1207fe77f30a8cdc673f9cabdf",
