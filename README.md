...
batch, and RIN must come from study metadata joined by sample ID; they are not
inferred from FASTQs or generated by the merge. Keep that metadata aligned to
the matrix sample order before downstream normalization or DA modeling.
